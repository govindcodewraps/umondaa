...
import 'package:hardware_lo/data_model/classified_ads_response.dart';
import 'package:hardware_lo/data_model/customer_package_response.dart';
import 'package:hardware_lo/data_model/user_info_response.dart';
...
import 'package:hardware_lo/screens/package/packages.dart';
import 'package:hardware_lo/ui_elements/classified_product_card.dart';
...
class _MyClassifiedAdsState extends State<MyClassifiedAds> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          elevation: 0,
          title: Text(LangText(context).local.my_products_ucf,style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold,color: MyTheme.dark_font_grey),),
      ...
      children: [
        Container(
          margin: EdgeInsets.symmetric(horizontal: 18),
            ...
            child: InkWell(
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context)=>UpdatePackage())).then((value) {
                  resetAll();
                });
  ...
  showDeleteWarningDialog(id){
    showDialog(context: context, builder: (context)=>Container(
      ...
      child: AlertDialog(
        title: Text(LangText(context).local.do_you_want_to_delete_it,style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.red),),
        content: Text(LangText( context).local.do_you_want_to_delete_it,style:const TextStyle(fontWeight: FontWeight.w400,fontSize: 14),),
        actions: [
          Btn.basic(
              color: MyTheme.accent_color,
              onPressed: (){
                Navigator.pop(context);
              }, child: Text(LangText( context).local.no_ucf,style: TextStyle(color: MyTheme.white,fontSize: 12),)),
          Btn.basic(
              color: MyTheme.accent_color,
              onPressed: (){
                Navigator.pop(context);
                 deleteProduct(id);
              }, child: Text(LangText(context).local.yes_ucf,style: TextStyle(color: MyTheme.white,fontSize: 12))),
        ],
      ),
    ));
  }
