...
import 'package:hardware_lo/custom/fade_network_image.dart';
import 'package:hardware_lo/custom/lang_text.dart';
import 'package:hardware_lo/custom/scroll_to_hide_widget.dart';
import 'package:hardware_lo/custom/useful_elements.dart';
import 'package:hardware_lo/data_model/carriers_response.dart';
import 'package:hardware_lo/data_model/delivery_info_response.dart';
import 'package:hardware_lo/helpers/system_config.dart';
import 'package:hardware_lo/repositories/cart_repository.dart';
import 'package:hardware_lo/repositories/pickup_points_repository.dart';
import 'package:hardware_lo/repositories/shipping_repository.dart';
...
import 'package:hardware_lo/data_model/city_response.dart';
import 'package:hardware_lo/data_model/country_response.dart';
...
/*
  ...
  onPressProceed(context) async {
    ...
    if(_sellerWiseShippingOptionValidation.isNotEmpty &&  carrier_base_shipping.$){
      ToastComponent.showDialog(LangText(context).local.please_choose_valid_info,
  ...
  AppBar buildAppBar(BuildContext context) {
    return AppBar(
      ...
      title: Text(
        "${AppLocalizations
            .of(context)
            .shipping_cost_ucf} ${_shipping_cost_string}",
    ...
    return Container(
        ...
        child: Center(
            child: Text(
              AppLocalizations
                  .of(context)
                  .carrier_points_is_unavailable_ucf,
              style: TextStyle(color: MyTheme.font_grey),
            )));
  }
  ...
  Container buildAppbarTitle(BuildContext context) {
    return Container(
      ...
      child: Text(
        "${AppLocalizations
            .of(context)
            .shipping_cost_ucf} ${SystemConfig.systemCurrency !=null?
        _shipping_cost_string.replaceAll(SystemConfig.systemCurrency.code, SystemConfig.systemCurrency.symbol)
            :_shipping_cost_string}",
        style: TextStyle(
            fontSize: 16,
            color: MyTheme.dark_font_grey,
