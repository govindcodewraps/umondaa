...
import 'package:hardware_lo/repositories/customer_package_repository.dart';
import 'package:hardware_lo/repositories/offline_wallet_recharge_repository.dart';
...
import 'package:hardware_lo/repositories/file_repository.dart';
import 'package:hardware_lo/repositories/offline_payment_repository.dart';
...
class _OfflineState extends State<OfflineScreen> {
  ...
  onPressSubmit() async {
    ...
    if (_photo_path == "" || _photo_upload_id == 0) {
      ToastComponent.showDialog(
          AppLocalizations.of(context).photo_proof_is_necessary, gravity: Toast.center, duration: Toast.lengthLong);
      return;
    }
    loading();
    if (widget.offLinePaymentFor==PaymentFor.WalletRecharge) {
      ...
      if (submitResponse.result == false) {
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) {
          return UpdatePackage(go_back: false);
        }));
      }
    }
  }
  ...
  onPickPhoto(context) async {
    ...
    if (status.isDenied) {
      ...
      ToastComponent.showDialog(
          AppLocalizations.of(context).go_to_your_application_settings_and_give_photo_permission, gravity: Toast.center, duration: Toast.lengthLong);
      ...
      if (_photo_file == null) {
        ToastComponent.showDialog(
            AppLocalizations.of(context).no_file_is_chosen, gravity: Toast.center, duration: Toast.lengthLong);
        return;
      }
                    ...
                    child: Btn.basic(
                      ...
                      shape: RoundedRectangleBorder(
                          borderRadius:
                              const BorderRadius.all(Radius.circular(8.0))),
                      child: Text(
                        AppLocalizations.of(context).submit_ucf+"00",
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 14,
                            fontWeight: FontWeight.w600),
